<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="{row}">
          <span>{{ row.point[0] + ' , ' + row.point[1] + ' , ' + row.point[2] + ' , ' + row.point[3] }}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="网络状态"
...
export default {
  ...
  methods: {
    getList() {
      ...
      // this.list = []
      const devId = Math.floor(Math.random() * 10000000)
      const level = ['A', 'B']
      ...
      if (this.list.length === 0) {
        for (let i = 0; i < 20; i++) {
          ...
          dev.id = devId + i // 设备
          dev.model = Math.floor(Math.random() * 3) // 型号
          ...
          dev.battery = (Math.random() * 100).toFixed(2) // 电量
          this.list.push(dev)
        }
      }
